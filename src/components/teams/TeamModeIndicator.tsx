...
import { Box, Button, Menu, MenuItem, ListItemIcon, ListItemText, Typography, Divider, Avatar, Tooltip, Badge } from "@mui/material";
		...
		<Box>
			...
			>
				...
				{teams.map((team) => {
					...
					return (
						<MenuItem key={team._id} onClick={() => handleSelectTeam(team)} selected={isSelected} sx={{ py: 1 }}>
							<ListItemIcon>
								<Avatar
									sx={{
										width: 28,
				...
				<MenuItem onClick={handleManageTeams}>
					...
					<ListItemText primary="Gestionar equipos" primaryTypographyProps={{ variant: "body2" }} />
				</MenuItem>
			</Menu>
		</Box>
	);
