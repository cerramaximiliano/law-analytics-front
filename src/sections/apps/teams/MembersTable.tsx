...
export default function MembersTable({ onMemberUpdated }: MembersTableProps) {
	...
	const handleChangeRole = async (newRole: TeamRole) => {
		...
		try {
			const result = await dispatch(updateMemberRole(activeTeam._id, selectedMember.userId, newRole) as any);
	...
	const handleRemoveMember = async () => {
		...
		try {
			const result = await dispatch(removeMember(activeTeam._id, selectedMember.userId) as any);
	...
	return (
		<>
										...
										<Stack direction="row" alignItems="center" spacing={1.5}>
											...
											<Box>
												<Typography variant="body2" fontWeight={500}>
													{fullName || "Sin nombre"}
													{isCurrentUser && (
														<Chip label="Tú" size="small" color="primary" variant="outlined" sx={{ ml: 1, height: 20, fontSize: 10 }} />
													)}
												</Typography>
											</Box>
										</Stack>
									...
									<TableCell align="right">
										{canEdit && (
											<Tooltip title="Opciones">
												<IconButton size="small" onClick={(e) => handleMenuOpen(e, member as unknown as TeamMember)} disabled={isUpdating}>
			...
			<ConfirmDialog
				...
				content={`¿Estás seguro de que deseas remover a ${
					selectedMember?.firstName || selectedMember?.email
				} del equipo? Esta persona perderá acceso a todos los recursos compartidos.`}
				confirmText="Remover"
				confirmColor="error"
