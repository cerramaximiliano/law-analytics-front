...
const PostalTrackingDetail = ({ open, tracking, loading, handleClose }: Props) => {
	...
	return (
		...
		>
			...
			<Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ px: 3, borderBottom: `1px solid ${theme.palette.divider}` }}>
												...
												<TableBody>
													{[...history].reverse().map((event, idx) => (
														<TableRow key={idx} hover>
															<TableCell sx={{ whiteSpace: "nowrap" }}>{formatDate(event.eventDate)}</TableCell>
															<TableCell>
								...
								<Box sx={{ flex: 1, overflow: "auto", p: 2 }}>
									{attachmentBusy ? (
										...
										<Stack spacing={1.5} alignItems="flex-start">
											...
											<Button variant="outlined" size="small" href={tracking!.attachmentUrl!} target="_blank" rel="noopener noreferrer">
												Abrir PDF
											</Button>
										</Stack>
									) : (
