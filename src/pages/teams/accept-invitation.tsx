  ...
  const handleAccept = async (skipResourceCheck = false) => {
    ...
    try {
      ...
      // This happens when Redux shows isLoggedIn but the actual session has expired
        ...
        err.response?.data?.message?.includes("Datos insuficientes")
      ) {
      ...
      return (
        <Stack spacing={3}>
          ...
          <Stack spacing={2}>
            <Button
              variant="contained"
              onClick={() => navigate("/apps/profiles/account/role")}
        ...
        <Stack spacing={3}>
          ...
          <Alert severity="success" sx={{ py: 1 }}>
            ...
            <Typography variant="body2" sx={{ mt: 0.5 }}>
              Tus recursos se transferirán al equipo en estado archivado. No ocuparán cuota de recursos activos
              y podrás desarchivarlos cuando haya capacidad disponible.
            </Typography>
          </Alert>
          ...
          <Stack spacing={2}>
            <Button
              variant="outlined"
              onClick={() => setFlowState("handle-resources")}
            >
              Volver a opciones
            </Button>
            <Button
