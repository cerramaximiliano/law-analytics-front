...
const DocumentDetailDialog = ({ open, document, onClose }: DocumentDetailDialogProps) => {
	...
	const linkedFolder = document.linkedFolderId ? folders.find((f: FolderData) => f._id === document.linkedFolderId) : null;
	const linkedTracking =
		supportsTracking && document.linkedTrackingId ? allTrackings.find((t) => t._id === document.linkedTrackingId) : null;
					...
					<Grid item xs={12} md={8} sx={{ height: "100%", borderRight: 1, borderColor: "divider" }}>
						{document.documentUrl ? (
							...
							<Stack alignItems="center" justifyContent="center" sx={{ height: "100%" }}>
								<Typography color="textSecondary">El PDF aún no ha sido generado o no está disponible.</Typography>
							</Stack>
						)}
					</Grid>
...
const VincularDialog = ({ open, document, onClose, onSuccess, showSnackbar }: VincularDialogProps) => {
	...
	const handleSaveFolder = async () => {
		...
		try {
			await dispatch(
				updatePostalDocument(document._id, {
...
const DocumentsLayout = () => {
	...
	return (
		...
		>
										...
										>
											...
											<Stack direction="row" alignItems="flex-start" spacing={1} sx={{ px: 1.5, pt: 1.5, pb: 1 }}>
												...
												<Stack spacing={0.25} sx={{ flex: 1, minWidth: 0 }}>
													<Stack direction="row" alignItems="center" spacing={0.75} flexWrap="wrap">
														...
														{row.templateCategory && <Chip size="small" label={row.templateCategory} variant="outlined" />}
													</Stack>
											...
											<CardActions sx={{ px: 1, py: 0.5, justifyContent: "flex-end" }}>
												<Tooltip title="Ver documento">
													<IconButton size="small" onClick={() => handleViewDetail(row._id)} color="default" aria-label="Ver documento">
														<Eye size={18} />
													</IconButton>
												</Tooltip>
												{row.documentUrl && (
												...
												<Tooltip title="Vincular">
													<IconButton size="small" color="default" onClick={() => setVincularDialogDoc(row)} aria-label="Vincular">
														<FolderOpen size={18} />
													</IconButton>
												</Tooltip>
												<Tooltip title="Eliminar">
													<IconButton size="small" onClick={() => setDocumentToDelete(row)} color="error" aria-label="Eliminar">
														<Trash size={18} />
													</IconButton>
												</Tooltip>
											</CardActions>
			...
			{/* Modales */}
			<CreatePostalDocumentModal open={openCreate} handleClose={handleCloseCreate} showSnackbar={showSnackbar} />

			<DocumentDetailDialog open={detailOpen} document={documentDetail} onClose={handleCloseDetail} />
			...
			{documentToDelete && (
				<AlertPostalTrackingDelete numberId={documentToDelete.title} open={Boolean(documentToDelete)} handleClose={handleDeleteConfirm} />
			)}
