...
// Instancia propia de axios contra pjcatamarca-api (mismo patrón que EJE y MEV:
// el hub no proxea estas lecturas).
const pjcatamarcaAxios: AxiosInstance = axios.create({
	baseURL: PJCATAMARCA_BASE_URL,
	timeout: 30000,
...
export interface PjCatamarcaCausaResponse {
	success: boolean;
	message?: string;
	data?: any;
}

class PjCatamarcaWorkersService {
	/**
	 * Busca una causa ya cargada en el sistema por CUIJ.
	 *
	 * Sirve como preview antes de crear la carpeta, pero NO es el camino de
	 * alta: eso lo hace `POST /api/folders` del hub con `pjcatamarca: true`, que
	 ...
	 */
	async searchCausaByCuij(cuij: string): Promise<PjCatamarcaCausaResponse> {
		try {
			const response = await pjcatamarcaAxios.get(`/api/causas/cuij/${encodeURIComponent(cuij)}`);
	...
	validateCuij(cuij: string): { valid: boolean; error?: string } {
		...
		if (!/^J-\d{2}-\d+-\d+(?:\/\d{4}-\d+)?$/i.test(normalized)) {
			return { valid: false, error: "El CUIJ debe tener el formato J-03-00021075-6/2025-0" };
		}
...
export default new PjCatamarcaWorkersService();
