...
const richTextDocumentsReducer = (state = initialState, action: any): State => {
  switch (action.type) {
    ...
    case UPDATE_TEMPLATE:
      return { ...state, templates: state.templates.map(t => t._id === action.payload._id ? action.payload : t), template: action.payload, isLoader: false };
    case DELETE_TEMPLATE:
      return { ...state, templates: state.templates.filter(t => t._id !== action.payload) };
    case SET_DOCUMENTS:
      return { ...state, documents: action.payload.documents, documentsTotal: action.payload.total, isLoader: false };
    case SET_DOCUMENT:
    ...
    case UPDATE_DOCUMENT:
      return { ...state, documents: state.documents.map(d => d._id === action.payload._id ? action.payload : d), document: action.payload, isLoader: false };
...
export const fetchRichTextTemplates = (params: RichTextTemplatesQueryParams = {}) => async (dispatch: Dispatch) => {
...
export const fetchRichTextDocuments = (params: RichTextDocumentsQueryParams = {}) => async (dispatch: Dispatch) => {
  dispatch({ type: SET_LOADING });
  try {
