...
import {
	PostalTrackingType,
	PostalTrackingState,
	PostalTrackingFilters,
	CreatePostalTrackingData,
	UpdatePostalTrackingData,
} from "types/postal-tracking";
...
export const fetchPostalTrackings =
	(filters: PostalTrackingFilters = {}) =>
		...
		try {
			...
			const msg =
				error instanceof AxiosError
					? error.response?.data?.message || "Error al obtener los seguimientos"
					: "Error al obtener los seguimientos";
...
export const createPostalTracking = (data: CreatePostalTrackingData) => async (dispatch: Dispatch) => {
	try {
		...
		const msg =
			error instanceof AxiosError ? error.response?.data?.message || "Error al crear el seguimiento" : "Error al crear el seguimiento";
		dispatch({ type: SET_ERROR, payload: msg });
		return { success: false, error: msg };
	}
};
...
export const getPostalTrackingById = (id: string) => async (dispatch: Dispatch) => {
	try {
		...
		const msg =
			error instanceof AxiosError ? error.response?.data?.message || "Error al obtener el seguimiento" : "Error al obtener el seguimiento";
		dispatch({ type: SET_ERROR, payload: msg });
		return { success: false, error: msg };
	}
};
...
export const updatePostalTracking = (id: string, data: UpdatePostalTrackingData) => async (dispatch: Dispatch) => {
	try {
		...
		const msg =
			error instanceof AxiosError
				? error.response?.data?.message || "Error al actualizar el seguimiento"
...
export const linkFolderToTracking = (id: string, folderId: string | null) => async (dispatch: Dispatch) => {
	try {
		...
		const msg =
			error instanceof AxiosError ? error.response?.data?.message || "Error al vincular la carpeta" : "Error al vincular la carpeta";
		dispatch({ type: SET_ERROR, payload: msg });
		return { success: false, error: msg };
	}
};
...
export const reactivatePostalTracking = (id: string) => async (dispatch: Dispatch) => {
	try {
		...
		const msg =
			error instanceof AxiosError
				? error.response?.data?.message || "Error al reactivar el seguimiento"
				: "Error al reactivar el seguimiento";
		return { success: false, error: msg };
	}
};
...
export const markPostalTrackingAsCompleted = (id: string) => async (dispatch: Dispatch) => {
	try {
		...
		const msg =
			error instanceof AxiosError
				? error.response?.data?.message || "Error al completar el seguimiento"
...
export const bulkDeletePostalTrackings = (ids: string[]) => async (dispatch: Dispatch) => {
	try {
		...
		const msg =
			error instanceof AxiosError
				? error.response?.data?.message || "Error al eliminar los seguimientos"
				: "Error al eliminar los seguimientos";
		return { success: false, error: msg };
	}
};
...
export const deletePostalTracking = (id: string) => async (dispatch: Dispatch) => {
	try {
		...
		const msg =
			error instanceof AxiosError
				? error.response?.data?.message || "Error al eliminar el seguimiento"
...
export const deleteAttachment = (id: string) => async (_dispatch: Dispatch) => {
	try {
		...
		const msg =
			error instanceof AxiosError ? error.response?.data?.message || "Error al eliminar el adjunto" : "Error al eliminar el adjunto";
		return { success: false, error: msg };
	}
};
