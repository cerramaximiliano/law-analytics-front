...
export const getContactsByUserId =
	...
	async (dispatch: Dispatch, getState: any): Promise<{ success: boolean; contacts?: any; message?: string; archivedByFolderCount?: number }> => {
		try {
			...
			const fields = "_id,name,lastName,email,phone,phoneCelular,phoneCodArea,role,type,address,street,streetNumber,floor,apartment,city,state,zipCode,company,status,folderIds,importSource,intervinienteRef";
