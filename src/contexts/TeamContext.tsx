...
export function TeamProvider({ children }: TeamProviderProps) {
  ...
  const { teams, activeTeam, currentUserRole, isLoading, isInitialized, error } = useSelector(
    (state) => state.teams
  );
  ...
  const isCurrentUserOwner = useMemo(() => {
    ...
    const ownerId = typeof activeTeam.owner === 'string'
      ? activeTeam.owner
      : (activeTeam.owner as any)?._id;
  ...
  const setActiveTeam = useCallback(
    async (team: Team | null) => {
      if (team && currentUserId) {
        ...
        const ownerId = typeof team.owner === 'string'
          ? team.owner
...
export function useTeamsFeature() {
  ...
  const isTeamsEnabled = useMemo(() => {
    ...
    if (subscription.featuresWithDescriptions) {
      const teamsFeature = subscription.featuresWithDescriptions.find(
        (f: { name: string; enabled: boolean }) => f.name === "teams"
      );
  ...
  const maxTeamMembers = useMemo(() => {
    ...
    if (subscription.limitsWithDescriptions) {
      const teamMembersLimit = subscription.limitsWithDescriptions.find(
        (l: { name: string; limit: number }) => l.name === "teamMembers"
      );
      if (teamMembersLimit) {
